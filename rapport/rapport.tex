...
\caption{Sortie de l'algorithme pour la dimension 1}
\label{Sortie en 1D}
\end{center}
\end{figure}
...
Les zonotopes sont une classe de polygone ayant des propriétés particulières. Moins précises que les enveloppes convexes quand il s'agit d'approximer un ensemble quelconque, elles ont l'avantage d'etre très facile à stocker, et d'avoir des opérations faciles à calculer (somme de Minkowski, et application linéaire).En utilisant la somme de Minkowski, on peut obtenir simplement une approximation des valeurs prises par l'équation entre 0 et t, puis on peut obtenir en temps linéaire et en espace constant les valeurs prises par l'équation pour ttout les intervals successifs [n*t; n*(t+1)].
Une fois que nous sommes capable d'obtenir une sur-estimation de l'ensemble des valeurs prises par l'équation différentielles, pour un zonotope donné initial, il nous faut alors déterminer quand les guardes sont prises. Il n'est pas facile de calculer l'intersection entre un hyperplan et un zonotope. En revanche, il est facile de déterminer si l'intersection est non-nulle. On peut alors sur-approximer l'intersection d'un zonotope et d'un hyperplan au zonotope lui-même.
De cette façon, nous obtenons une méthode d'analyse n-dimensionnelle, beaucoup plus précise que les hypercubes, mais ayant un coût de calcul relativement faible.


\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=0.5]{zonotopes.png}
\caption{Sortie de l'algorithme utilisant les zonotopes}
\label{Sortie en ND}
\end{center}
\end{figure}
